...
final class BBCode {
	...
	public function bbcodeAllow(): array {
		return ['url', 'b', 'i', 'u', 's', 'font', 'size', 'color', 'center', 'align', 'spoiler', 'code', 'quote', 'video','sub', 'sup', 'image', 'item', 'list', 'kbd', 'table', 'thead', 'tbody', 'th', 'td', 'tr', 'notice', 'info', 'warning', 'error', 'success', 'highlight', 'mention', 'badge', 'diff', 'divider'];
	}
	 ...
	 */
	public function setRestriction(array $bbcodes = []): void {
	 ...
	 */
	private function unclosedTags(): void {
	 ...
	 * saltos de línea
	 */
	private function setExtraTags(): void {
	...
	private function registerBBCodes(): void {
		$args = [[true], [false]];
		$codeConfigs = [
	...
	private function getAdvancedTags(array $customValidators): array {
		return [
			...
			[
				'tag' => 'spoiler',
				'replace' => '<div class="spoiler"><div class="title" contenteditable="false"><a href="#" onclick="spoiler($(this)); return false;" nonce="'. CSP_NONCE .'">{option}</a></div><div class="body">{param}</div></div>',
				'option' => true,
			], [
				'tag' => 'spoiler',
	 ...
	 */
	public function addBBcodes(): void {
	...
	*/
	public function parseSmiles(): void {
		global $tsCore;
		// SMILEYS
		$bbcode = [];
		$html = [];
