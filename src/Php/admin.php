...
$ctx = Controller::init('admin', 'admin');
...
if($ctx->continue()) {
	...
	# CONFIGURACION DEL REGISTRO
		...
		$Mailer = Container::get(EmailConfig::class);
		...
		if(empty($act)) {
			...
			if($_SERVER['REQUEST_METHOD'] === 'POST') {
				...
				if(!preg_match('/^backup_[\w\-]+\.sql$/', $filename)) {
					Container::get(Redirector::class)->to("{$tsCore->settings['url']}/admin/dbmanager?act=historial");
					exit;
				}
	...
	# TEMAS
		...
		$smarty->assign("tsTemas", Container::get(tsThemes::class)->getTemas());

	# NOTICIAS
	} elseif($action === 'news') {
	    $tsNoticias = Container::get(tsNoticias::class);
	...
	# PUBLICIDADES
	    ...
	    if (empty($act)) {
	        $smarty->assign('tsAds', Ads::getAll());
	    } elseif ($act === 'editar') {
	    	$id = (int)($_GET['id'] ?? 0);
	        if (!empty($_POST['titulo'])) {
	            if (Ads::save($id)) {
	                $tsCore->redirectAdmin($action, 'save', '&act=editar&id=' . $id);
	            }
	        }
	        $smarty->assign('tsAd', Ads::get($id));
	    }

	# MEDALLAS
	} elseif($action === 'medals') {
		# Incluimos el archivo
	    $tsMedallas = Container::get(tsMedal::class);
	...
	# ESTADÍSTICAS
	    ...
	    $tsStats = Container::get(tsEstadisticas::class);
		$smarty->assign("tsAdminStats", $tsStats->obtenerEstadisticas());

	# LISTA NEGRA
	} elseif($action === 'blacklist') {
	    $tsBloqueos = Container::get(tsBloqueos::class);
	...
	# CENSURAS
	    ...
	    $tsCensura = Container::get(tsCensura::class);
	...
	# CATEGORIAS
	    ...
	    $tsCategorias = Container::get(tsCategorias::class);
		if($act === 'editar' || $act === 'nueva') {
			if(isset($_POST['c_nombre'])) {
				$status = $act === 'editar' ? $tsCategorias->saveCat() : $tsCategorias->newCat();
				if($status) $tsCore->redirectAdmin($action);
			} else {
			...
			if(isset($_POST['save'])) {
				if($tsCategorias->MoveCat()) $tsCore->redirectAdmin($action);
			}
		} elseif($act === 'borrar'){
			if(isset($_POST['save'])) {
				if($tsCategorias->delCat()) $tsCore->redirectAdmin($action);
	...
	# RANGOS
	    ...
	    $tsRangos = Container::get(tsRangos::class);
	    ...
	    $tsUserAdmin = Container::get(tsUserAdmin::class);
