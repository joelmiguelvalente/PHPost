...
$IP = Container::get(IP::class)->getIPBinary();
$tsEmail = Container::get(Email::class);
	
$email = Html::escape($_REQUEST['r_email']);
$user_info = DB::fetch("SELECT user_id, user_name, user_registro, user_activo FROM u_miembros WHERE user_email = :email", ['email' => $email]);
if(!$user_info){
	...
	case 'recover-validation':
		...
		<div style="background:#0f7dc1;padding:10px;font-family:Arial, Helvetica,sans-serif;color:#000">
			...
			<div style="background:#FFF;padding:10px;font-size:16px">
				...
				<p>Para finalizar con el proceso de registro, confirma tu dirección de email accediendo a <a href="{$Routes->route('url')}/validar/$pinHash/2/{$tsData['user_email']}">este enlace</a> y luego ingresando este pin <strong>$hash</strong>
				</p>
