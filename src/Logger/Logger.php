...
final class Logger {
	...
	public static function warning(string $msg, array $ctx = [], ?string $channel = null): void {
		self::log($msg, LogLevel::WARNING, $ctx, $channel);
	}

	public static function error(string $msg, array $ctx = [], ?string $channel = null): void {
		self::log($msg, LogLevel::ERROR, $ctx, $channel);
	}

	public static function info(string $msg, array $ctx = [], ?string $channel = null): void {
		self::log($msg, LogLevel::INFO, $ctx, $channel);
	}

	public static function debug(string $msg, array $ctx = [], ?string $channel = null): void {
		self::log($msg, LogLevel::DEBUG, $ctx, $channel);
	}

	private static function buildFilePath(string $channel): string {
		$now = new DateTimeImmutable('now', new DateTimeZone(date_default_timezone_get()));
		$filenameDate = $now->format('d_m_Y');
		$dir = Config::app('paths.logs.full_path');
		if (!is_dir($dir)) {
			mkdir($dir, 0777, true);
		}
