...
class tsDenuncias {
	...
	public function __construct(
		...
		// VARS
		$this->razon = $this->Core->setSecure(trim($_POST['razon'] ?? ''));
		$this->extras = $this->Core->setSecure(trim($_POST['extras'] ?? ''));
	}
	...
	private function setDenunciaPost(int $objId): string {
      ...
      if ($denuncias >= 2) {
         DB::update('p_posts', ['post_status' => 1], 'post_id = :id', ['id' => $objId]);
	...
	/*
		setDenuncia()
	*/
	public function setDenuncia(int $objId, string $type = 'post') {
		return match($type) {
