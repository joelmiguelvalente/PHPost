...
class tsMuro {
	...
	*/
	...
	private function streamPostVideo(string $attachment, int $pid, string $body, int $date): array|string {
	   ...
	   $this->User->antiFlood();
	   $pubId = $this->MuroHelper->insertMuro($pid, Html::escape($body, true), 4, $date);
	   if (!$pubId) {
	...
	/* 
		streamPost()
	*/
	public function streamPost(): string|array {
		//
		$pid 			= (int)($_POST['pid'] ?? 0);
		$data 		= Html::escape($_POST['data'], true);
		$attachment = Html::escape($_POST['adj'], true);
		$type 		= Html::escape(trim($_GET['type'] ?? ''));
		$date			= time();
		...
		$privacidad = $this->getPrivacity(
			(int)$pid,
			$exists,
			(int)$this->Cuenta->isFollowed($pid, true),
			(int)$this->Cuenta->isFollowed($pid, false)
		);
	...
	/*
		streamRepost()
	*/
	public function streamRepost(): string|array {
		global $tsMonitor, $tsActividad;
		//
		$data = Html::escape($this->Core->parseBadWords($_POST['data']), true);
		$pid = (int)($_POST['pid'] ?? 0);
		$date = time();
		$pub = DB::fetch("SELECT p_user, p_user_pub FROM u_muro WHERE pub_id = :pid LIMIT 1", ['pid' => $pid]);
		//
		if((int)$pub['p_user'] === 0) {
			return '0: La publicación no existe.';
		}
		$text = str_replace(["\n", "\t", ' '], '', $data);
		if ($text === '') {
	...
	private function loadNewsWall(string $query, array $param): array {
	 ...
	 * @return array
	 **/
	public function getNews(int $start = 0, int $limit = 10): array {
	...
	*/
	public function getStory(int $pubId, int $userId): string|array {
		// ELEGIMOS
		$pub = DB::fetch("SELECT p.*, u.user_id, u.user_name FROM u_muro AS p LEFT JOIN u_miembros AS u ON p.p_user_pub = u.user_id WHERE p.pub_id = :pubId LIMIT 1", ['pubId' => $pubId]);
		// COMPROBAMOS
		if($pub['pub_id'] === '') return 'La publicación que has solicitado no existe.';
		elseif($userId !== (int)$pub['p_user']) return "La publicación que has solicitado no pertenece al perfil de <strong>{$this->User->getUserName($userId)}</strong>.";
		// CARGAR LIKES
		...
		// ADJUNTOS
		if((int)$pub['p_type'] !== 1) {
			$attachment = DB::fetch("SELECT * FROM u_muro_adjuntos WHERE pub_id = :pubId LIMIT 1", ['pubId' => $pubId]);
	...
	*/
	public function getComments(): string|array {
		$pid = (int) ($_POST['pid'] ?? 0);
		// EXISTE?
		$comment = DB::fetch("SELECT `p_user`, `p_comments` FROM `u_muro` WHERE `pub_id` = :pid LIMIT 1", ['pid' => $pid]);
		//
		if(empty($comment)) return '0: La publicación no existe.';
	...
	public function deletePost(): string {
		...
		switch($type) {
			case 'pub':
				...
				//
				if($data['p_user'] === '') return '0: La publicación no existe.';
				...
				DB::delete('u_muro', 'pub_id = :id', ['id' => $id]);
				// BORRAMOS LOS LIKES DE LA PUBLICACION
				DB::delete('u_muro_likes', 'obj_id = :id AND obj_type = 1', ['id' => $id]);
				// BORRAMOS LOS LIKES DE TODOS LOS COMENTARIOS
				$comments = DB::fetchAll("SELECT `cid` FROM `u_muro_comentarios` WHERE `pub_id` = :id", ['id' => $id]);
				// IDS A BORRAR
				$ids = [];
				...
				if (!empty($ids)) {
					...
					DB::query("DELETE FROM `u_muro_likes` WHERE `obj_id` IN ({$inClause}) AND `obj_type` = :obj_type", $params);
				}
				// BORRAR COMENTARIOS
				DB::delete('u_muro_comentarios', 'pub_id = :id', ['id' => $id]);
			...
			case 'cmt':
				...
				if(DB::delete('u_muro_comentarios', 'cid = :id', ['id' => $id]) === 0) {
					return '0: Error al eliminar comentario';
				}
				// UPDATES
				DB::delete('u_muro_likes', 'obj_id = :id AND obj_type = 2', ['id' => $id]);
	...
	*/
	public function likePost(): array {
		global $tsMonitor, $tsActividad;
		// ANTI FLOOD
		$text = $this->User->antiFlood(false, 'like', 'No te pueden gustar tantas cosas en tan poco tiempo.');
		...
		// EXISTE O NO
		if ($type === 1) {
			$exists = DB::fetch("SELECT p_user AS uid FROM u_muro WHERE pub_id = :id", ['id' => $id]);
		} else {
			$exists = DB::fetch("SELECT c_user AS uid FROM u_muro_comentarios WHERE cid = :id", ['id' => $id]);
		}
		
		if(empty($exists) || $exists['uid'] === '') return '0: La publicación ya no existe.';
		// CHECAMOS SI YA LE GUSTA ESTO
		$likes = DB::fetchAll("SELECT like_id, user_id FROM u_muro_likes WHERE obj_id = :id AND obj_type = :type", ['id' => $id, 'type' => $type]);
		$total = count($likes ?? 0);
		// CHECAMOS
		$iLike = 0;
		...
		// SI AUN NO ME GUSTA
		if(empty($iLike)){
			if(DB::insert('u_muro_likes', ['user_id' => $this->User->uid, 'obj_id' => $id, 'obj_type' => $type])) {
				// SUMAR LIKE
				if($type === 1) {
					DB::increment('u_muro', 'p_likes', 'pub_id = :id', ['id' => $id]);
					$ac_type = ((int)$exists['uid'] === $this->User->uid) ? 0 : 2; 
				} else {
					DB::increment('u_muro_comentarios', 'c_likes', 'cid = :id', ['id' => $id]);
			...
			if(DB::delete('u_muro_likes', 'like_id = :id', ['id' => $iLike])){
				// RESTAR LIKE
				if($type === 1) {
					DB::decrement('u_muro', 'p_likes', 'pub_id = :id', ['id' => $id]);
				} else {
					DB::decrement('u_muro_comentarios', 'c_likes', 'cid = :id', ['id' => $id]);
				}
			} else $status = 'error';
		}
		// RESPUESTA
	...
	*/
	public function showLikes(): array {
		$id = (int)($_POST['id'] ?? 0);
		$type = ($_POST['type'] === 'com') ? 2 : 1;
		$data = DB::fetchAll("SELECT l.user_id, u.user_name FROM u_muro_likes AS l LEFT JOIN u_miembros AS u ON l.user_id = u.user_id WHERE obj_id = :id AND obj_type = :type", ['id' => $id, 'type' => $type]);
		if(empty($data)) return ['status' => 0, 'data' => 'La publicación no existe.'];
