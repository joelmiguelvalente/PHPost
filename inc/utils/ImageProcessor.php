...
class ImageProcessor
{
	...
	private const DEFAULT_CONFIG = [
		...
		'formats' => ['webp', 'avif'],   // Formatos de salida soportados
	...
	private function mergeConfig(array $config): array
	{
		...
		if (empty($merged['temp_dir'])) {
			$merged['temp_dir'] = dirname(__DIR__, 2) . '/temp/';
		}
	...
	private function validateImage(string $file): void
	{
		...
		if ($size === false || $size <= 0) {
			throw new \RuntimeException("Archivo vacío o corrupto");
		}

		// Validar MIME type
		$finfo = finfo_open(FILEINFO_MIME_TYPE);
		if (!$finfo) {
			throw new \RuntimeException("No se pudo inicializar finfo");
		}
		...
		if ($dimensions[0] * $dimensions[1] > $maxPixels) {
			throw new \RuntimeException("Resolución de imagen excesiva");
		}
	}
	...
	private function convertToWebp(string $source, string $destination): void
	{
		...
		if (!imagewebp($image, $destination, $this->config['quality'])) {
			imagedestroy($image);
			throw new \RuntimeException("Error al convertir a WebP");
		}
		imagedestroy($image);
	}
	...
	private function logError(string $message): void
	{
		if ($this->config['log_errors']) {
			error_log("[ImageProcessor] {$message}");
		}
	}
}