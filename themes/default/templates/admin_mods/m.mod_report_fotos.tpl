                                ...
                                {if $tsSave}<div style="display: block;" class="mensajes ok">Tus cambios han sido guardados.</div>{/if}
                                        ...
                                        <tbody>
                                            ...
                                            <tr id="report_{$r.foto_id}">
                                                ...
                                                <td class="admin_actions">
                                                    ...
                                                    {if $tsUser->is_admod || $tsUser->permisos.mocdf}<a href="#" onclick="mod.reboot({$r.foto_id}, 'fotos', 'reboot', false); return false;"><img src="{$tsRoutes.tema.images}/icons/reboot.png" title="{if $r.f_status == 1}Reactivar Foto{else}Desechar denuncias{/if}" /></a>{/if}
                                                    {if $tsUser->is_admod || $tsUser->permisos.moedfo}<a href="{$tsConfig.url}/fotos/editar.php?id={$r.foto_id}" target="_blank"><img src="{$tsRoutes.tema.images}/icons/edit.png" title="Editar Foto" /></a>{/if}
                                                    {if $tsUser->is_admod || $tsUser->permisos.moef}<a href="#" onclick="mod.fotos.borrar({$r.foto_id}); return false"><img src="{$tsRoutes.tema.images}/icons/close.png" title="Borrar Foto" /></a>{/if}
                                                </td>
                                            </tr>
                                            {/foreach}{else}
                                            <tr>
                                    ...
                                    <h2 style="border-bottom:1px dashed #CCC; padding-bottom:5px;">
                                        ...
                                        <span class="floatR admin_actions">
                                            {if $tsUser->is_admod || $tsUser->permisos.mocdf}<a href="#" onclick="mod.reboot({$tsDenuncia.data.foto_id}, 'fotos', 'reboot', true); return false"><img src="{$tsRoutes.tema.images}/icons/reboot.png" title="{if $tsDenuncia.data.f_status == 1}Reactivar Fotos{else}Desechar denuncias{/if}" /></a>{/if}
                                            {if $tsUser->is_admod || $tsUser->permisos.moedfo}<a href="{$tsConfig.url}/fotos/editar.php?id={$tsDenuncia.data.foto_id}" target="_blank"><img src="{$tsRoutes.tema.images}/icons/edit.png" title="Editar Fotos" /></a>{/if}
                                            {if $tsUser->is_admod || $tsUser->permisos.moef}<a href="#" onclick="mod.fotos.borrar({$tsDenuncia.data.foto_id}); return false"><img src="{$tsRoutes.tema.images}/icons/close.png" title="Borrar Foto" /></a>{/if}
                                        </span>
                                    </h2>
                                    <table cellpadding="0" cellspacing="0" border="0" class="admin_table" width="100%" align="center">
                                    	<thead>
