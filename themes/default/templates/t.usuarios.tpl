	...
	<div class="filterBy filterFull" style="width:925px">
		<div class="block floatL" style="line-height:25px">
			...
			<ul> 
				...
				<li{if $tsFiltro.sex == 'male'} class="here"{/if}><label><input type="radio" name="sexo" value="male" {if $tsFiltro.sex == 'male'}checked="true"{/if}/>Hombre</label></li> 
				<li{if $tsFiltro.sex == 'female'} class="here"{/if}><label><input type="radio" name="sexo" value="female" {if $tsFiltro.sex == 'female'}checked="true"{/if}/> Mujer</label></li> 
				<li{if $tsFiltro.sex == 'none'} class="here"{/if}><label><input type="radio" name="sexo" value="none" {if $tsFiltro.sex == 'none'}checked="true"{/if}/>Ambos</label></li>
	...
	<div id="showResult" class="resultFull" style="float:left; width:755px">
			...
			{if $tsUsers}
				...
				<li class="resultBox clearfix">
					<h4 style="padding:0"><span class="rango qtip" style="background-image:url({$tsRoutes.tema.images}/icons/ran/{$u.rango.image});" title="{$u.rango.title}">&nbsp;</span> <a href="{$tsConfig.url}/perfil/{$u.user_name}" style="color:#{$u.rango.color}" >{$u.user_name}</a></h4>
					<div class="floatL avatarBox" style="padding:0">
						<a href="{$tsConfig.url}/perfil/{$u.user_name}"><img width="75" height="75" src="{$tsRoutes.storage.avatar}/avatar_120.webp" class="av"/></a>
					</div>
