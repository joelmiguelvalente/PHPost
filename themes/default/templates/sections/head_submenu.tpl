<div class="subMenuContent">
	<div id="subMenuPosts" class="subMenu {if $tsPage != 'tops'}here{/if}">
		<ul class="floatL tabsMenu">
			...
			{if $tsUser->is_member}
			{if $tsUser->is_admod || $tsUser->can('gopp')}<li{if $tsSubmenu == 'agregar'} class="here"{/if}><a class=vctip title="Agregar Post" href="{$tsConfig.url}/agregar/">Agregar Post</a></li>{/if}
			<li class="{if $tsPage == 'mod-history'}here{/if}"><a class=vctip title="Historial de Moderaci&oacute;n" href="{$tsConfig.url}/mod-history/">Historial</a></li>
				{if $tsUser->is_admod || $tsUser->can('moacp')}
	...
	<div id="subMenuFotos" class="subMenu {if $tsPage == 'fotos'}here{/if}">
		<ul class="floatL tabsMenu">
			...
			{if $tsUser->is_admod || $tsUser->can('gopf')}<li{if $tsAction == 'agregar'} class="here"{/if}><a href="{$tsConfig.url}/fotos/agregar.php">Agregar Foto</a></li>{/if}
