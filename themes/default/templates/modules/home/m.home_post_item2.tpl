...
<div class="listPost-item slim{if $data.post_sponsored == 1} patrocinado{/if}{if $data.post_status >= 1 || $data.post_status <= 3} post_status{$data.post_status}{/if}{if $data.user_activo == 0} user_activo{/if}{if $data.user_baneado == 1} user_baneado{/if}">
	...
	<div class="item-data">
		<h4><a class="item-title" href="{$tsConfig.url}/posts/{$data.c_seo}/{$data.post_id}/{$data.post_title|seo}.html" title="{if $data.post_status == 3}El post est&aacute; en revisi&oacute;n{elseif $data.post_status == 1}El post se encuentra en revisi&oacute;n por acumulaci&oacute;n de denuncias{elseif $data.post_status == 2}El post est&aacute; eliminado{elseif $data.user_activo == 0}La cuenta del usuario est&aacute; desactivada{else}{$data.post_title}{/if}"  target="_self">{$data.post_title}</a></h4>
