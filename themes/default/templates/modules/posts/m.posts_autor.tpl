	...
	<div class="box_cuerpo">
		<div class="avatarBox">
			<a href="{$tsConfig.url}/perfil/{$tsAutor.user_name}">
				<img title="Ver perfil de {$tsAutor.user_name}" alt="Ver perfil de {$tsAutor.user_name}" class="avatar" src="{$tsRoutes.storage.avatar}/avatar_{$tsAutor.user_id}.webp"/>
			</a>
		</div>
...
<img src="{$tsRoutes.tema.images}/icons/med/{$m.m_image}_16.png" style="margin-left:1px;margin-bottom:2px;" class="qtip" title="{$m.m_title} - {$m.m_description}"/>
	...
	<ul style="margin-left:11px;">
		{foreach from=$tsPost.visitas item=v}
		 <a href="{$tsConfig.url}/perfil/{$v.user_name}" class="hovercard" uid="{$v.user_id}" style="display:inline-block;"><img src="{$tsRoutes.storage.avatar}/avatar_50.webp" class="vctip" title="{$v.date|hace:true}" width="32" height="32"/></a> 
		{/foreach}
	</ul>
	</div>
	  {/if}
