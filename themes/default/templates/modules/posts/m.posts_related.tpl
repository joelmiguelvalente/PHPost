		...
		{if $tsRelated}
			{foreach from=$tsRelated item=p}
				<div class="categoriaPost p-1 mb-2">
					...
					<a href="{$tsConfig.url}/posts/{$p.c_seo}" class="flex justify-start items-center gap-1 font-bold"><img src="{$tsRoutes['assets:images']}/icons/categories/{$p.c_img}" alt="{$p.c_nombre}"> {$p.c_nombre}</a>
				</div>
			{/foreach}
		{else}
