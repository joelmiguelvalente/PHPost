<div style="--image:100px;display:grid;grid-template-columns:var(--image) 1fr;gap:1rem;padding:1rem 0">
	<a href="{$tsConfig.url}/afiliado-url?ref={$tsAfiliado.aid}" onclick="dialog.close()" target="_blank" style="display:block;width:var(--image);height:var(--image);overflow:hidden;border-radius:.325rem">
	...
	<div style="display: flex;justify-content:center;align-items:flex-start;flex-direction: column;">
		<small><a href="{$tsConfig.url}/afiliado-url?ref={$tsAfiliado.aid}" onclick="dialog.close()" target="_blank">{$tsAfiliado.a_url}</a></small>
