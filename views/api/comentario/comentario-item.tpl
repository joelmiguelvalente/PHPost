...
<li class="comment-item{if $tsPost.autor == $c.c_user} author{elseif $c.c_user == $tsUser->uid} owner{/if}" data-comment-id="{$c.cid}">
	<a class="comment-avatar" aria-label="{$c.user_name} " href="{$tsConfig.url}/perfil/{$c.user_name}">
	...
	<div class="comment-body">
		...
		{/if}
		{if $c.replies}
			<ul class="replies-container" id="replies-{$c.cid}" style="display:none;" role="list">
