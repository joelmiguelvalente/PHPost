								...
								<table cellpadding="0" cellspacing="0" border="0" class="admin_table" width="100%" align="center">
									...
									<tbody>
										...
										<tr id="report_{$p.post_id}">
											...
											<td><a href="{$tsConfig.url}/@{$p.user_name}" class="hovercard" uid="{$p.user_id}">{$p.user_name}</a></td>
											<td><a href="{$tsConfig.url}/@{$p.mod_name}" class="hovercard" uid="{$p.mod}">{$p.mod_name}</a></td>
									...
									<tfoot>
										<td colspan="7">Páginas: {$tsPospelera.pages}</td>
									</tfoot>
								</table>
								{/if}
                                </div>
