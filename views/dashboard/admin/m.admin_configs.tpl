	...
	<form method="POST" autocomplete="off" class="space-y-6">
		<fieldset class="rounded-lg border border-gray-200 dark:border-gray-700 p-6 bg-white dark:bg-surface shadow-sm">
		   ...
		   <div class="grid grid-cols-1 md:grid-cols-3 gap-4 py-2 mb-3">
			...
			{include "dashboard/FormGroup.tpl" type="radio" id="c_allow_upload" label="Carga externa" helper="Si cuentas con un servidor de pago o la librer&iacute;a CURL puedes subir im&aacute;genes remotamente a imgur.com" name="c_allow_upload" checked=$tsConfig.c_allow_upload|default:'0' labels=["Sí", "No"] values=[1,0]}

			{include "dashboard/FormGroup.tpl" type="radio" id="c_allow_portal" label="Activar portal" helper="Los usuarios podr&aacute;n tener un inicio perzonalizado." name="c_allow_portal" checked=$tsConfig.c_allow_portal|default:'0' labels=["Sí", "No"] values=[1,0]}
			
			{include "dashboard/FormGroup.tpl" type="radio" id="c_fotos_private" label="Secci&oacute;n de fotos oculta" helper="Si est&aacute; activado, los visitantes no podr&aacute;n ver la secci&oacute;n fotos." name="c_fotos_private" checked=$tsConfig.c_fotos_private|default:'0' labels=["Sí", "No"] values=[1,0]}
			
			{include "dashboard/FormGroup.tpl" type="radio" id="c_see_mod" label="Vista moderativa amplia" helper="Si est&aacute; activado, el equipo de moderaci&oacute;n podr&aacute; ver, diferenciado por colores, los distintos estados de los posts." name="c_see_mod" checked=$tsConfig.c_see_mod|default:'0' labels=["Sí", "No"] values=[1,0]}
			
			{include "dashboard/FormGroup.tpl" type="radio" id="c_desapprove_post" label="Revisi&oacute;n de posts tras su publicaci&oacute;n" helper="Si est&aacute; activado, el equipo de moderaci&oacute;n deber&aacute; aprobar un post antes de que &eacute;ste sea publicado." name="c_desapprove_post" checked=$tsConfig.c_desapprove_post|default:'0' labels=["Sí", "No"] values=[1,0]}
			...
			{include "dashboard/FormGroup.tpl" type="radio" id="c_allow_live" label="Notificaciones Live" helper="Los usuarios podr&aacute;n ver en tiempo real sus notificaciones. (Esta opci&oacute;n puede consumir un poco m&aacute;s de recursos.)" name="c_allow_live" checked=$tsConfig.c_allow_live|default:'0' labels=["Sí", "No"] values=[1,0]}
			...
			{include "dashboard/FormGroup.tpl" type="radio" id="c_allow_sump" label="Los votos suman puntos" helper="Cada voto positivo en un comentario es un punto m&aacute;s para el usuario. <strong>Nota:</strong> Los votos negativos no restan puntos" name="c_allow_sump" checked=$tsConfig.c_allow_sump|default:'0' labels=["Sí", "No"] values=[1,0]}
